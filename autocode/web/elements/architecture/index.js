...
export class ArchitectureDashboard extends LitElement {
     ...
     */
    _getPathSegments() {
        if (!this._snapshot?.nodes) return [{ id: '.', name: '🏗️ root' }];
        ...
        while (currentId != null) {
            ...
            segments.unshift({
                id: node.id,
                name: node.id === this._snapshot.root_id ? '🏗️ root' : node.name,
            });
            currentId = node.parent_id;
        }

        return segments.length > 0 ? segments : [{ id: '.', name: '🏗️ root' }];
    }
    ...
    _renderSnapshotInfo(snap) {
        return html`
            <div class="snapshot-info">
                🏗️ Commit ${snap.commit_short} · ${snap.branch} · ${this._formatDate(snap.timestamp)}
            </div>
        `;
    }
...
if (!customElements.get('architecture-dashboard')) {
    customElements.define('architecture-dashboard', ArchitectureDashboard);
}
